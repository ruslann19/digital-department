...
def index(request):
    ...
    html += '<ul>'
    html += f'<li><a href="{projects_list_url}">Список проектов</a></li>'
    quality_control_url = reverse('quality_control:index')
    html += f'<li><a href="{quality_control_url}">Контроль качества</a></li>'
    html += '</ul>'
...
def project_detail(request, project_id):
    ...
    response_html = f'<h1>{project.name}</h1>'
    response_html += f'<p>{project.description}</p>'
...
class TaskDetailView(DetailView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        response_html = f'<h1>{task.name}</h1>'
        response_html += f'<p>{task.description}</p>'
        return HttpResponse(response_html)
